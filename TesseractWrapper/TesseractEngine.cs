...
namespace TesseractWrapper
{
    ...
    public class TesseractEngine : DisposableBase
    {
        ...
        #region Constructors
        /// <summary>
        /// Creates a new instance of <see cref="TesseractEngine"/> using the <see cref="OcrEngineMode.Default"/> mode.
        /// </summary>
        /// <remarks>
        /// <para>
        ...
        public TesseractEngine(string datapath, string language)
            : this(datapath, language, OcrEngineMode.Default, new string[0], new Dictionary<string, object>(), false)
        {
        }

        /// <summary>
        /// Creates a new instance of <see cref="TesseractEngine"/> with the specified <paramref name="configFile"/>
        /// using the <see cref="OcrEngineMode.Default">Default Engine Mode</see>.
        /// </summary>
        /// <remarks>
        /// <para>
        /// The <paramref name="datapath"/> parameter should point to the directory that contains the 'tessdata' folder
        ...
        public TesseractEngine(string datapath, string language, string configFile)
            : this(datapath, language, OcrEngineMode.Default, configFile != null ? new[] { configFile } : new string[0], new Dictionary<string, object>(), false)
        {
        }
        ...
        public TesseractEngine(string datapath, string language, IEnumerable<string> configFiles)
            : this(datapath, language, OcrEngineMode.Default, configFiles, new Dictionary<string, object>(), false)
        {
        }
        ...
        /// <param name="datapath">The path to the parent directory that contains the 'tessdata' directory, ignored if the <c>TESSDATA_PREFIX</c> environment variable is defined.</param>
        /// <param name="language">The language to load, for example 'eng' for English.</param>
        /// <param name="engineMode">The <see cref="OcrEngineMode"/> value to use when initialising the tesseract engine.</param>
        public TesseractEngine(string datapath, string language, OcrEngineMode engineMode)
        ...
        /// with Unix end of line characters you can use an advanced text editor such as Notepad++ to accomplish this.
        /// </param>
        public TesseractEngine(string datapath, string language, OcrEngineMode engineMode, string configFile)
        ...
        /// with Unix end of line characters you can use an advanced text editor such as Notepad++ to accomplish this.
        /// </param>
        public TesseractEngine(string datapath, string language, OcrEngineMode engineMode, IEnumerable<string> configFiles)
        ...
        /// with Unix end of line characters you can use an advanced text editor such as Notepad++ to accomplish this.
        /// </param>
        public TesseractEngine(string datapath, string language, OcrEngineMode engineMode, IEnumerable<string> configFiles, IDictionary<string, object> initialOptions, bool setOnlyNonDebugVariables)
        {
        ...
        /// <returns></returns>
        private Page Process(Bitmap image, PageSegMode? pageSegMode = null)
        {
        ...
        /// <param name="pageSegMode">The page segmentation mode.</param>
        /// <returns></returns>
        private Page Process(Bitmap image, string inputName, PageSegMode? pageSegMode = null)
        {
            return Process(image, inputName, new Rect(0, 0, image.Width, image.Height), pageSegMode);
        }
        ...
        /// <param name="pageSegMode">The page segmentation mode.</param>
        /// <returns></returns>
        private Page Process(Bitmap image, Rect region, PageSegMode? pageSegMode = null)
        {
            return Process(image, null, region, pageSegMode);
        }
        ...
        /// <param name="pageSegMode">The page segmentation mode.</param>
        /// <returns></returns>
        private Page Process(Bitmap image, string inputName, Rect region, PageSegMode? pageSegMode = null)
        {
        ...
        public bool ProcessImage(string filename,int time, HandleRef renderer)
        {
            if (renderer.Handle == IntPtr.Zero)
                throw new ExecutionEngineException("Rendere has not been initialized!");
            if (string.IsNullOrEmpty(filename))
                throw new ArgumentException("Specify filename!");
            return Interop.TessApi.BaseAPIProcessPages(Handle, filename, dataPath, time, renderer) == 0 ? true : false;
        }
        ...
        private void Initialise(string datapath, string language, OcrEngineMode engineMode, 
            IEnumerable<string> configFiles = null, IDictionary<string, object> initialValues = null, bool setOnlyNonDebugVariables = false)
        {
            Guard.RequireNotNullOrEmpty("language", language);
            Guard.RequireNotNull("handle", handle);
            ...
            string[] configFileArray = null;
            int configFileSize = 0;

            if (configFiles != null)
            {
                configFileArray = new List<string>(configFiles).ToArray();
                configFileSize = configFileArray.Length;
            }

            string[] keyStrs = null;
            string[] valStr = null;
            int valCount = initialValues == null ? 0 : initialValues.Count;
#warning FTW!!!
            //Internal.TessHelper.IDictionaryToTessParams(initialValues, out keyStrs, out valStr);

            if (Interop.TessApi.BaseApiInit(handle, datapath, language, (int)engineMode, configFileArray, configFileSize, keyStrs, valStr, (UIntPtr)valCount, setOnlyNonDebugVariables) != 0)
            {
        ...
        /// <summary>
        /// Gets or sets default <see cref="PageSegMode" /> mode used by <see cref="TesseractWrapper.TesseractEngine.Process(Pix, Rect, PageSegMode?)" />.
        /// </summary>
        ...
        {
            value = Interop.MarshalHelper.PtrToString(Interop.TessApi.BaseApiGetStringVariable(handle, name), Encoding.UTF8);
            return value != null;
        }
        #endregion Config
