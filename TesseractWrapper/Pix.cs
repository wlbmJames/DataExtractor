...
namespace TesseractWrapper
{
    public unsafe sealed class Pix : DisposableBase
    {
        ...
        public static Pix LoadTiffFromMemory(byte[] bytes)
        {
            ...
            {
                handle = Interop.LeptonicaApi.pixReadMemTiff(ptr, bytes.Length, 0);
            }
        ...
        public int XRes
        {
            get { return Interop.LeptonicaApi.pixGetXRes(this.handle); }
            set { Interop.LeptonicaApi.pixSetXRes(this.handle, value); }
        }

        public int YRes
        {
            get { return Interop.LeptonicaApi.pixGetYRes(this.handle); }
        ...
        //    int same;
        //    if(Interop.LeptonicaApi.pixEqual(Handle, other.Handle, out same) != 0)
        //    {
        ...
        public Pix BinarizeOtsuAdaptiveThreshold(int sx, int sy, int smoothx, int smoothy, float scorefract)
        {
            ...
            if (ppixth != IntPtr.Zero)
            {
                ...
                Interop.LeptonicaApi.pixDestroy(ref ppixth);
            }
        ...
        public Pix BinarizeSauvola(int whsize, float factor, bool addborder)
        {
            ...
            int result = Interop.LeptonicaApi.pixSauvolaBinarize(handle, whsize, factor, addborder ? 1 : 0, out ppixm, out ppixsd, out ppixth, out ppixd);

            // Free memory held by other unused pix's

            if (ppixm != IntPtr.Zero)
            {
                Interop.LeptonicaApi.pixDestroy(ref ppixm);
            }

            if (ppixsd != IntPtr.Zero)
            {
                Interop.LeptonicaApi.pixDestroy(ref ppixsd);
            }
        ...
        public Pix BinarizeSauvolaTiled(int whsize, float factor, int nx, int ny)
        {
            ...
            IntPtr ppixth, ppixd;
            int result = Interop.LeptonicaApi.pixSauvolaBinarizeTiled(handle, whsize, factor, nx, ny, out ppixth, out ppixd);
        ...
        //        pix1 = Interop.LeptonicaApi.pixThresholdToBinary(handle, 170);
        ...
        //        if (pix1 != IntPtr.Zero)
        //        {
        //            Interop.LeptonicaApi.pixDestroy(ref pix1);
        //        }

        //        if (pix2 != IntPtr.Zero)
        //        {
        //            Interop.LeptonicaApi.pixDestroy(ref pix2);
        //        }

        //        if (pix3 != IntPtr.Zero)
        //        {
        //            Interop.LeptonicaApi.pixDestroy(ref pix3);
        //        }

        //        if (pix4 != IntPtr.Zero)
        //        {
        //            Interop.LeptonicaApi.pixDestroy(ref pix4);
        //        }

        //        if (pix5 != IntPtr.Zero)
        //        {
        //            Interop.LeptonicaApi.pixDestroy(ref pix5);
        //        }

        //        if (pix6 != IntPtr.Zero)
        //        {
        //            Interop.LeptonicaApi.pixDestroy(ref pix6);
        //        }

        //        if (pix7 != IntPtr.Zero)
        //        {
        //            Interop.LeptonicaApi.pixDestroy(ref pix7);
        //        }

        //        if (pix9 != IntPtr.Zero)
        //        {
        //            Interop.LeptonicaApi.pixDestroy(ref pix9);
        //        }
        //    }
        //}
        ...
        public Pix Rotate(float angleInRadians, RotationMethod method = RotationMethod.AreaMap, RotationFill fillColor = RotationFill.White, int? width = null, int? height = null)
        {
            ...
            {
                // handle general case
                resultHandle = Interop.LeptonicaApi.pixRotate(handle, angleInRadians, method, fillColor, width.Value, height.Value);
            }
        ...
        //{
        //    IntPtr resultHandle = Interop.LeptonicaApi.pixRotate90(handle, direction);
        ...
        {
            IntPtr result = Interop.LeptonicaApi.pixScale(handle, scaleX, scaleY);
